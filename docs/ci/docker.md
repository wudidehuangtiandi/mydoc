...
```shell
redis 注意配置文件默认为空，需要去[地址](https://github.com/redis/redis/blob/unstable/redis.conf)搞一个,不过这个配置文件可能存在版本问题下面贴一个通用的 。注意，用配置文件需要修改三处保护模式(protected-mode)修改为no。
...
#
requirepass wudidehuangtiandi123
